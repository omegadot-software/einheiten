...
export const UNITS: IRegularObject<UnitDefinition> = {
	...
	"<beerbarrel>": [
		["bl", "bl-us", "beer-barrel", "beer-barrels"],
		0.1173477658,
		"volume",
		["<meter>", "<meter>", "<meter>"],
	],
	"<beerbarrel-imp>": [
		["blimp", "bl-imp", "beer-barrel-imp", "beer-barrels-imp"],
		0.16365924,
		"volume",
		["<meter>", "<meter>", "<meter>"],
	],
	...
	"<kelvin>": [["K", "degK", "kelvin", "Kelvin"], 1.0, "temperature", ["<kelvin>"]],
	"<celsius>": [
		["°C", "ºC", "degC", "celsius", "Celsius", "centigrade", "Centigrade"],
		1.0,
		"temperature",
		["<kelvin>"],
	],
	"<fahrenheit>": [
		["°F", "ºF", "degF", "fahrenheit", "Fahrenheit"],
		5 / 9,
		"temperature",
		["<kelvin>"],
	],
	"<rankine>": [["°R", "degR", "rankine"], 5 / 9, "temperature", ["<kelvin>"]],
...
function validateUnitDefinition(unitDef: string, definition: UnitDefinition) {
	...
	numerator.forEach((unit) => {
		if (UNITS[unit] === undefined) {
			throw new QtyError(
				`${unitDef}: Invalid unit definition. ` + `Unit ${unit} in 'numerator' is not recognized`
			);
		}
	});
	...
	denominator.forEach((unit) => {
		if (UNITS[unit] === undefined) {
			throw new QtyError(
				`${unitDef}: Invalid unit definition. ` + `Unit ${unit} in 'denominator' is not recognized`
			);
		}
	});
}

export const PREFIX_VALUES: IRegularObject<number> = {};
export const PREFIX_MAP: IRegularObject<string> = {};
export const UNIT_VALUES: IRegularObject<IScalarAndUnit> = {};
export const UNIT_MAP: IRegularObject<string> = {};
export const OUTPUT_MAP: IRegularObject<string> = {};
